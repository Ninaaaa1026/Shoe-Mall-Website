...
<body onload="sort()">
<%session.setAttribute("formerUrl","customerOrder");%>
			...
			<c:if test="${ISLOGIN=='1'}">
				<div id="dingdan">
					<table class="pure-table pure-table-horizontal"
						...
						<tbody>
							<c:forEach var="no" items="${requestScope.OrderNo}" begin="0">
								<tr style="font-size:1.1em;">
									<td>${no.date}</td>
									<td>订单号：
									<a href="Order?action=searchName&name=${LoginName}">${no.orderNumber}</a>
									</td>
									<td>${no.paymentStatus}</td>
									<td>${no.orderStatus}</td>
									<td>${no.deliveryState}</td>
									<td>运费：${no.freight}</td>
									<td></td>
									<td></td>
								</tr>
								<c:forEach var="item" items="${requestScope.Saleses}">
									<c:if test="${no.orderNumber==item.orderNumber}">
